...
set :linked_dirs, %w{log tmp/pids tmp/cache tmp/sockets vendor/bundle public/system public/upload}
...
#       execute "#{fetch(:sudo)} ln -nfs #{fetch(:release_path)}/config/unicorn_init.sh /etc/init.d/unicorn_#{fetch(:application)}"
